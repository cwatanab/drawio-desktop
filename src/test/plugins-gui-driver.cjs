...
module.exports = async ({win, js, fs, pause}) =>
{
	...
	await key('Right');
	await check('document.activeElement.dataset.quickStyler', 'autosize', 'Right opens property list');
	await key('Down');
	await check('document.activeElement.dataset.quickStyler', 'aspect', 'Down moves through properties');
	await key('Down');
	await check('document.activeElement.dataset.quickStyler', 'noLabel', 'Down moves past divider to next group');
	await key('Up');
	await key('Up');
	await check('document.activeElement.dataset.quickStyler', 'autosize', 'Up returns to autosize');
	await key('Enter');
